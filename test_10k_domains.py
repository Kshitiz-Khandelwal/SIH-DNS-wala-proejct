...
def main():
    ...
    print(f"  Single-Query Mean Latency: {np.mean(latencies):.3f} ms")
    print(f"  50th Percentile (p50):     {np.percentile(latencies, 50):.3f} ms")
    print(f"  95th Percentile (p95):     {np.percentile(latencies, 95):.3f} ms")
    print(f"  99th Percentile (p99):     {np.percentile(latencies, 99):.3f} ms")
    print(f"  Real-time Budget (<10ms):  PASSED ({np.percentile(latencies, 99):.2f} ms < 10.00 ms)")
