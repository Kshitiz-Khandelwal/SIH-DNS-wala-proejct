...
out=Path("artifacts");out.mkdir(exist_ok=True);joblib.dump(model,out/f"{a.name}-v1.joblib");(out/f"{a.name}-v1.metrics.json").write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
